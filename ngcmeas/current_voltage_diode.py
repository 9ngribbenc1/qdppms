...
class myKeithley6221(Keithley6221):
    ...
    def diode_current_sweep_setup(self, maxI, crit_curr,crit_curr_window, crit_numpoints, out_numpoints, sweeptype, delay, nplc, pulse_width):
        print('in diode_current_sweep_setup')
        # This is based on 4-9 of the KE6221 user manual,
        # and pulsed delta section
        ...
        if sweeptype == 'list':
            ...
            crit_upper = crit_curr + crit_curr_window
            crit_lower = crit_curr - crit_curr_window

            out_up_neg = np.linspace(-maxI, -crit_upper, out_numpoints)
            crit_up_neg = np.linspace(-crit_upper, -crit_lower, crit_numpoints)
            low_up = np.linspace(-crit_lower, crit_lower, out_numpoints)
            crit_up_pos = np.linspace(crit_lower, crit_upper, crit_numpoints)
            out_up_pos = np.linspace(crit_upper, maxI, out_numpoints)

            out_dn_pos = np.linspace(maxI, crit_upper, out_numpoints)
            crit_dn_pos = np.linspace(crit_upper, crit_lower, crit_numpoints)
            low_dn = np.linspace(crit_lower, -crit_lower, out_numpoints)
            crit_dn_neg = np.linspace(-crit_lower, -crit_upper, crit_numpoints)
            out_dn_neg = np.linspace(-crit_upper, -maxI, out_numpoints)

            crnts = np.concatenate((out_up_neg, crit_up_neg,low_up,crit_up_pos,out_up_pos,out_dn_pos,crit_dn_pos,low_dn,crit_dn_neg,out_dn_neg))
    ...
    def find_IC(self):
        print('in Find_IC')
        IC=0
        ICdata = pd.DataFrame({'Current': [abs(i) for i in self.curr], \
                             'Voltage': [abs(i) for i in self.volt]})
        ICdata.sort_values(by=['Current'], inplace=True)
        print(ICdata)
        delt=0
        for i in range(len(ICdata['Current'])-3):
            if abs((ICdata['Voltage'][i]) - ICdata['Voltage'][i+1])>delt and ICdata['Current'][i]<ICdata['Current'][i+1]:
                delt=abs((ICdata['Voltage'][i]) - ICdata['Voltage'][i+1])
                IC=ICdata['Current'][i+1]
        print(IC)
        return(IC)
